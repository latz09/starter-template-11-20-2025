			...
			<div className='space-y-0.5'>
				...
				<Step number={1} title='Create a new Sanity project'>
					...
					<Note>
						...
						Note your <strong>Project ID</strong> — you will need it for env
						vars.
					</Note>
				</Step>
				...
				<Step number={2} title='Update sanity.config.js (full version)'>
					...
					<Note>
						Customize your theme at <strong>sanity.build/themer</strong>, then
						replace the URL above with the new hues before committing.
					</Note>
				</Step>
				...
				<Step number={3} title='Add custom Studio branding'>
					...
					<CodeBlock
						language='components/clientLogo.jsx'
						code={clientLogoCode}
					/>
					<Note>
						Drop the client&apos;s logo at{' '}
						<code>public/static/client-logo.png</code> in the Studio project. 25
						× 25 px works well; use a square or icon version of their mark. File
						must be <code>.jsx</code> — JSX won&apos;t parse in a plain{' '}
						<code>.js</code> file.
					</Note>
					<Label>components/CustomLayout.jsx</Label>
					<CodeBlock
						language='components/CustomLayout.jsx'
						code={customLayoutCode}
					/>
					<Note>
						This adds the &ldquo;Powered by Latz Web Design&rdquo; footer with a
						click-to-copy email popup. Paste in the full component code from
						your template.
					</Note>
				</Step>
				...
				<Step number={5} title='Add the homepage schema'>
					<Note>
						Create <code>schemaTypes/homePage.js</code> — pre-filled with
						placeholder content so you can test fetching immediately without
						typing anything.
					</Note>
					...
					/>
					<Note>
						In Studio, create a new Home Page document — it will pre-fill
						automatically. Publish it, then confirm it fetches in your Next.js
						app.
					</Note>
				</Step>
				...
				<Step number={6} title='Add the navigation schema'>
					...
					/>
					<Note>
						Register all three schemas in <code>schemaTypes/index.js</code>:
					</Note>
				...
				{/* STEP 7 */}
				<Step number={7} title='Fill out .env.local (Next.js project)'>
					<Note>
				...
				{/* STEP 8 */}
				<Step number={8} title='Fill out .env (Sanity Studio project)'>
					<Note>
				...
				{/* STEP 9 */}
				<Step number={9} title='Start both dev servers'>
				...
				{/* STEP 10 */}
				<Step number={10} title='Test draft mode'>
					<Note>
						The draft mode route is already in the starter at{' '}
						<code>app/api/draft-mode/enable/route.js</code>. Just make sure{' '}
						<code>SANITY_VIEWER_TOKEN</code> is set in <code>.env.local</code>,
						then hit this URL to confirm it&apos;s working:
					</Note>
				...
				{/* STEP 11 */}
				<Step number={11} title='Configure Desk Structure'>
					<Note>
						Set up <code>structure.js</code> based on the pages and singletons
						this project needs. This is manual and changes per project — there
						is no universal template. Define singleton documents (Home Page,
